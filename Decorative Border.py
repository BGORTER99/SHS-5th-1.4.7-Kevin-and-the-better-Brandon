...
def frame(original_image, color, percent_of_side, randpic=1):
    ...
    if randpic==1:
        random_frame = random.choice(file_list)
    else:
        random_frame = os.path.join(os.getcwd(), 'frame.jpg')
    ...
    if use_decorative_frame: 
        frame_pic = PIL.Image.open(random_frame)
...
def frame_all_images(directory=None, color=(255,0,0), width=0.10, randwid=1):
    ...
    for n in range(len(image_list)):
        ...
        if randwid==1:
            width = float(decimal.Decimal(random.randrange(0,20))/100)
        if randwid==1:
            new_image = frame(image_list[n],color, width, randpic=1)
        else:
            new_image = frame(image_list[n],color, width, randpic=0)
